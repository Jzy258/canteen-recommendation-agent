...
def test_category_search():
    r = search_dish.invoke({"keyword": "素菜"})
    assert len(r) > 0, "Should find 素菜 dishes"
    print(f"  category '素菜': {len(r)} results")


def test_tag_search():
    r = search_dish.invoke({"keyword": "清淡"})
    assert len(r) > 0, "Should find 清淡 dishes"
    print(f"  tag '清淡': {len(r)} results")


def test_all_dishes_count():
    r = get_all_dishes.invoke({})
    assert len(r) == 30, f"Should have 30 dishes, got {len(r)}"
