...
schema = open(os.path.join(_PROJECT_ROOT, "backend", "db", "schema.sql"),
              encoding="utf-8").read()
check("schema 版本 v1.2", "v1.2" in schema)
check("dish 含 serving_grams", "serving_grams" in schema and "standard" in schema.replace("标准", "standard"))
...
print("=" * 55)
print("交付物 7: record.py 工具支持 grams")
print("=" * 55)
rec_code = open(os.path.join(_PROJECT_ROOT, "backend", "tools", "record.py"),
                encoding="utf-8").read()
check("record_meal 支持 grams", "grams" in rec_code)
