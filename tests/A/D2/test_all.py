...
menu_path = "backend/data/menu.csv"
...
# dish_id 全部可映射到 dishes.csv
dish_path = "backend/data/dishes.csv"
with open(dish_path, encoding="utf-8") as f:
    dish_ids = {int(r["id"]) for r in csv.DictReader(f)} if "id" in csv.DictReader(f).fieldnames else set()

# 如果 dishes.csv 没有 id 列，计算行号作为 ID
if not dish_ids:
    with open(dish_path, encoding="utf-8") as f:
        dish_ids = set(range(1, len(list(csv.DictReader(f))) + 1))

for r in menu_rows:
    did = int(r["dish_id"])
    assert did in dish_ids, f"dish_id {did} 在 dishes.csv 中不存在"
print(f"  [PASS] 所有 dish_id 有效 (范围 1-{len(dish_ids)})")
...
clean_path = "backend/data/clean.py"
...
log_path = "backend/data/cleaning_log.md"
...
with open("backend/data/init_db.py", encoding="utf-8") as f:
    init_code = f.read()
